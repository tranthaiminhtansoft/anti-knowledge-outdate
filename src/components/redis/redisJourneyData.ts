...
export const redisChapters: RedisChapter[] = [
  ...
  },
  {
    ...
    steps:[
      {title:'Logical endpoint',text:'Application dùng protocol/client được sản phẩm hỗ trợ mà không giữ slot map trực tiếp.',activeNodes:['app','proxy'],activeEdges:['e1'],metrics:{endpoint:'single',client_mode:'standard',shards:'3'},log:'rediss://database-endpoint:12000'},
  ...
  },
  {
    ...
    steps:[
      ...
      {title:'Security baseline',text:'Không expose Redis trực tiếp. Bắt buộc TLS, ACL least privilege, network isolation, secret rotation và audit; replication qua WAN phải dùng encrypted transport.',activeNodes:['topology','responsibility'],activeEdges:['e5','e6','e7','e8'],tones:{responsibility:'warn'},metrics:{tls:'required',acl:'least privilege',secret_rotation:'scheduled'},log:'rediss:// endpoint\nnetwork policy: restricted\naudit + rotation enabled'}
    ]
  },
  {
    ...
    steps:[
      ...
      {title:'SLOWLOG và commandstats',text:'Tìm expensive command/complexity; SLOWLOG chủ yếu đo thời gian command thực thi phía Redis, không bao gồm network I/O và thời gian client nhận payload, nên phải correlate với client latency/network. Không chạy MONITOR lâu trên production.',activeNodes:['alert','slowlog'],activeEdges:['e2'],tones:{slowlog:'warn'},metrics:{slowlog_len:'128',top_command:'HGETALL',server_duration:'412 ms'},log:'SLOWLOG GET 20\nHGETALL user_sessions_big'},
      ...
      {title:'Mitigate rồi sửa gốc',text:'Ưu tiên rate-limit, đổi command, cô lập traffic hoặc phân mảnh key. Chỉ UNLINK sau khi xác nhận key là cache tái tạo được, có approval và rollback; theo dõi DB/cache miss rồi lập permanent fix.',activeNodes:['keys','infra','action'],activeEdges:['e6','e7'],tones:{action:'good'},metrics:{p99:'24 ms',cpu:'48%',rollback:'ready'},log:'rate limit applied\ncache key verified rebuildable\napproved UNLINK\np99 recovered'}
    ]
  }
];
